...
public class SubmitScoreHandlerTests
{
    ...
    public async Task Handle_NegativeReactionTime_ReturnsError()
    {
        ...
        // Assert
        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("inhuman speed");
    }
    ...
    [Fact]
    public async Task Handle_SuspiciouslyFastTimes_ReturnsError()
    {
        // Arrange: Times < 100ms are rejected as inhuman
        var command = new SubmitScore.Command(
            Nickname: "FastPlayer",
            AverageMs: 75.0,
        ...
        // Assert: Submission is rejected (inhuman speed detection)
        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("inhuman speed");
    }
